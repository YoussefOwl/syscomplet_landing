...
export class AjoutFournisseurComponent {
  @Output("FermerPopUp") FermerPopUp = new EventEmitter(); // EventEmitter<boolean> = new EventEmitter<boolean>();
  is_loading: boolean;
  /* ------------------------------ subscriptions ----------------------------- */
    ...
    if (copie_multimedia?.length > 0) {
      if (copie_multimedia[0].size > this.max_val_accepted) {
        ...
        reader.onload = () => {
          Promise.resolve()
          .then(() => {
            formData.append('libelle_fournisseur', this.FormulaireAjout.value?.libelle_fournisseur);
          ...
          .then(()=> this.AjouterFournisseur(formData))
        }
      }
    }
  ...
  AjouterFournisseur(data:any) {
    ...
    this.Ajouter = this.fournisseurService.AjouterFournisseur(data)
    .subscribe({
      next: (res:any)=> {
        switch(res?.api_message) {
          case "success":
            this.generalService
            .sweetAlert2('Fournisseur ajoutée avec succès', null, 'simple', 'success')
