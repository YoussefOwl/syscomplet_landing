...
export class FournisseurComponent {
    ...
    this.menu_items = [
      {
        ...
        route: '/fournisseurs/liste_fournisseur',
      }
    ];
  }
  ...
  getCurrentmenu() {
    const routeMap = {};
    this.menu_items.forEach((res) => { routeMap[res.route] = res?.label; })
    const route = this.router.url;
    const label = routeMap[route] || "Liste fournisseurs";
    this.items = [{ label, link: route }];
    this.InitSelected(this.menu_items.findIndex((item: any) => item?.route == route));
  }
