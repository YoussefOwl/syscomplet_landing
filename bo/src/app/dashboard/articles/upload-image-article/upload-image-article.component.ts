...
@Component({
  selector: 'app-upload-image-article',
  templateUrl: './upload-image-article.component.html'
})
export class UploadImageArticleComponent {
  ...
  ManageImage(data: any) {
    ...
    this.Upload = this.articleService.ManageImage(data)
      .subscribe({
        next: (res: any) => {
          switch (res?.api_message) {
            case "success":
              this.generalService
                .sweetAlert2('Image téléchargée avec succès', null, 'simple', 'success')
                .then(() => this.ClosePopUp(false));
              break;
            case "file_error":
              this.generalService.sweetAlert2("Erreur lors du téléchargement du fichier", null, 'simple', 'error');
              break;
            case "error":
